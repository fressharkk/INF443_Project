...
cgp::vec2 Dragon::get_neck_angles()
{
	float angle1;
	float angle2;
	rotation_transform::convert_quaternion_to_axis_angle(dragon_hierarchy["Neck Low"].transform.rotation.data, vec3(), angle1);
	rotation_transform::convert_quaternion_to_axis_angle(dragon_hierarchy["Neck Head"].transform.rotation.data, vec3(), angle2);
...
void Dragon::throw_fire(float t, float beginning_time, float duration, Flamme* flame, scene_environment_camera_head& environment)
{
	...
	flame->setTimeOfCreation(beginning_time + 0.6);
	environment.update_flame(vec4(flame->getConeRadius(), flame->getConeHeight(), flame->getSpeed(), flame->getTimeOfCreation()));
...
void Dragon::orienterBas(float t, float beginning_time, float duration)
{
	if (t < beginning_time)
		return;
	if (t - beginning_time > duration)
		return;
	if (t - beginning_time < 0.01)
		neck_angles = get_neck_angles();

	float target_angle_1 = Pi / 6;
	float target_angle_2 = Pi / 3;
	if (t - beginning_time < duration / 10)
	{
		dragon_hierarchy["Neck Low"].transform.rotation =  rotation_transform::from_axis_angle({-1,0,0}, std::abs(neck_angles.x) + (target_angle_1 - std::abs(neck_angles.x)));
		dragon_hierarchy["Neck Head"].transform.rotation= rotation_transform::from_axis_angle({ 1,0,0 }, std::abs(neck_angles.y) + (target_angle_2 - std::abs(neck_angles.y)));
	}
	else
	{
		dragon_hierarchy["Neck Low"].transform.rotation = rotation_transform::from_axis_angle({ -1,0,0 }, target_angle_1);
		dragon_hierarchy["Neck Head"].transform.rotation = rotation_transform::from_axis_angle({ 1,0,0 }, target_angle_2);
	}

}

void Flamme::setTimeOfCreation(float t)
{
	time_begin = t;
}
...
cgp::vec3 Flamme::getPos()
{
	return pos;
}

cgp::vec3 Flamme::getDirAxis()
{
	return dir_axis;
}

float Flamme::getConeRadius()
{
	return cone_radius;
}

float Flamme::getConeHeight()
{
	return cone_height;
}

float Flamme::getSpeed()
{
	return speed;
}

float Flamme::getTimeOfCreation()
{
	return time_begin;
}
