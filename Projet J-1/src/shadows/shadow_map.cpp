...
void shadow_map_structure::draw_with_shadow(cgp::hierarchy_mesh_drawable const& drawable_hier, scene_environment_shadow_map const& scene)
{
	// Setup shader
	assert_cgp(drawable_hier.elements[0].drawable.texture != 0, "Try to draw mesh_drawable without texture");
	...
	// Send uniforms for this shader
	opengl_uniform_shadow(shader_shadow_render, scene);
	for (auto d : drawable_hier.elements)
	{
		auto drawable = d.drawable;
		drawable.transform.translation = vec3(0,0,0);
		opengl_uniform(shader_shadow_render, drawable.shading);
		opengl_uniform(shader_shadow_render, "model", drawable.transform.matrix());
...
void shadow_map_structure::draw_shadow_map(cgp::hierarchy_mesh_drawable const& drawable_hier, scene_environment_shadow_map const& current_scene)
{
	GLuint shader = shader_shadow_map;
	glUseProgram(shader); opengl_check;
