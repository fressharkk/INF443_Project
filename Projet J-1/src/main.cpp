...
void mouse_move_callback(GLFWwindow* /*window*/, double xpos, double ypos)
{
	...
	if (scene.inputs.keyboard.shift)
		scene.mouse_camera2(scene.inputs.mouse.position.previous, scene.inputs.mouse.position.current);
	else if (scene.inputs.mouse.click.left)
	{
		scene.mouse_camera2(scene.inputs.mouse.position.previous, scene.inputs.mouse.position.current);
	...
	if (!scene.inputs.keyboard.shift)
		 camera_standard_behavior_rotation(scene.environment.camera_fixe, scene.inputs);
...
void mouse_click_callback(GLFWwindow* /*window*/, int button, int action, int /*mods*/)
{
	if (scene.inputs.mouse.click.right)
		scene.get_pointed_object(scene.inputs.mouse.position.current, scene.environment.camera);
